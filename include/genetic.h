...
/**
 * @brief Fit either a regressor, classifier or a transformer to the given
 * dataset
 *
 ...
 * @param params          host struct containing hyperparameters needed for
 * training
 * @param final_progs     device pointer to the final generation of
 * programs(sorted by decreasing fitness)
 * @param history         host vector containing the list of all programs in
 * every generation (sorted by decreasing fitness)
 *
 * @note This module allocates extra device memory for the nodes of the last
 * generation that is pointed by `final_progs[i].nodes` for each program `i` in
 * `final_progs`. The amount of memory allocated is found at runtime, and is
 * `final_progs[i].len * sizeof(node)` for each program `i`. The reason this
 * isn't deallocated within the function is because the resulting memory is
 * needed for executing predictions in `symRegPredict`, `symClfPredict`,
 * `symClfPredictProbs` and `symTransform` functions. The above device memory is
 ...
 * transformer(like sigmoid) is specified, then it is applied on the output
 * before returning it.
 *
 * @param input       device pointer to feature matrix
 * @param n_rows      number of rows of the feature matrix
 * @param params      host struct containing training hyperparameters
 * @param best_prog   The best program obtained during training. Inferences are
 * made using this
...
/**
 * @brief Return predictions for a binary classification program defining the
 * decision boundary
 *
 * @param input       device pointer to feature matrix
 * @param n_rows      number of rows of the feature matrix
